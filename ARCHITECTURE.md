...
incoming message (text)
  ...
  -> Entity.decay_all()  (per-emotion exponential decay: base_rate x category_modifier)
...
### 2. Emotion decay = base rate x category modifier

The final `decay_rate` baked into each `Emotion` instance at construction is:

```
_score_to_decay(personality_score) x DECAY_CATEGORY_MODIFIER.get(emotion_name, 1.0)
```
...
**How it works:** `compute_deltas(event, entity, witness_event=None)` accepts the sender's `last_event` as a third argument. This is already computed; no extra LLM call. Witness deltas are scaled at 0.55x severity and fire based on what the sender experienced, regardless of how they phrased their response.
